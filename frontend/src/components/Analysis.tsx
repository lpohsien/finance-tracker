    ...
    <div className="space-y-4 animate-in fade-in duration-300">
      <div className="relative">
        ...
        <input 
          ...
          className="w-full pl-10 pr-4 py-3 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-sm focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900 outline-none transition-all text-sm dark:text-white"
          value={searchTerm}
            ...
            filtered.map((t: any) => (
              ...
              >
                <div className="flex items-center space-x-3 overflow-hidden">
                   <div className="w-10 h-10 rounded-xl bg-gray-50 dark:bg-slate-800 flex items-center justify-center text-lg shrink-0 dark:text-gray-200">
                    {t.category ? t.category[0].toUpperCase() : '?'}
                   </div>
                   <div className="min-w-0">
                     <p className="font-semibold text-sm truncate dark:text-white">{t.description || t.bank}</p>
                     <p className="text-[10px] text-gray-400">{t.category} • {new Date(t.timestamp).toLocaleDateString()} {new Date(t.timestamp).toLocaleTimeString()}</p>
                   </div>
                </div>
                <div className="flex items-center space-x-3 shrink-0">
                  <p className={`font-bold text-sm ${t.amount < 0 ? 'text-red-500' : 'text-green-500'}`}>
