...
export default function Layout({ children, activeTab, onTabChange, onLogout }: LayoutProps) {
  ...
  const addTransactionMutation = useMutation({
    mutationFn: async (parsedData: { amount: number; category: string; note: string }) => {
        return api.post('/api/transactions', {
            type: 'expense',
            amount: parsedData.amount,
            description: parsedData.note || parsedData.category,
            bank: 'Manual',
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans pb-24 md:pb-0 md:flex md:flex-row">
      
      {/* Sidebar for Desktop */}
      <aside className="hidden md:flex flex-col w-64 border-r border-gray-200 bg-white/80 backdrop-blur-xl h-screen sticky top-0">
         <div className="p-8">
      ...
      <main className="flex-1 max-w-md mx-auto md:max-w-4xl md:mx-0 w-full relative">
        {/* Mobile Header */}
        <header className="md:hidden px-6 pt-12 pb-4 bg-white/80 backdrop-blur-md sticky top-0 z-10 border-b border-gray-100">
           <div className="flex justify-between items-end">
             <div>
                <p className="text-gray-500 text-sm font-medium uppercase tracking-wider">{new Date().toLocaleString('default', { month: 'long', year: 'numeric' })}</p>
        ...
        <header className="hidden md:block px-8 py-6 border-b border-gray-100 bg-white/50 backdrop-blur sticky top-0 z-10">
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-xl border-t border-gray-200 flex justify-around py-3 px-4 z-50 md:hidden">
        {navItems.map((item) => (
             <button 
             key={item.id}
             onClick={() => onTabChange(item.id)}
             className={`flex flex-col items-center space-y-1 transition-colors ${activeTab === item.id ? 'text-blue-600' : 'text-gray-400'}`}
           >
             <item.icon size={22} />
